...
from database import get_db  # sekarang pakai PostgreSQL
...
async def input_alamat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    cursor.execute("""
        INSERT INTO santri_data_pribadi (
            santri_nama_id, nis, nik, nomor_kk, tempat_lahir, tanggal_lahir, jenis_kelamin,
            agama, anak_ke, provinsi, kabupaten, kecamatan, alamat
        ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    """, (
        data['santri_nama_id'], data['nis'], data['nik'], data['nomor_kk'],
        data['tempat_lahir'], data['tanggal_lahir'], data['jenis_kelamin'],
        data['agama'], data['anak_ke'], data['provinsi'], data['kabupaten'],
        data['kecamatan'], data['alamat']
    ))
    conn.commit()
    conn.close()
