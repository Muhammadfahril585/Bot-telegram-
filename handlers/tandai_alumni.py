...
async def simpan_alumni(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur = conn.cursor()
    cur.execute("SELECT nama_lengkap FROM santri_nama WHERE id = ?", (santri_id,))
    result = cur.fetchone()

    if result:
        nama = result[0]
        cur.execute("INSERT INTO alumni (santri_nama_id, nama_lengkap, tahun_lulus) VALUES (?, ?, ?)", (santri_id, nama, int(tahun)))
        conn.commit()
        await update.message.reply_text(f"✅ *{nama}* berhasil ditandai sebagai Alumni {tahun}.", parse_mode="Markdown")
    else:
