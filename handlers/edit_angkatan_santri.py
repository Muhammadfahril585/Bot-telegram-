...
async def simpan_edit_angkatan(update: Update, context: CallbackContext):
    ...
    c.execute("SELECT id FROM angkatan WHERE santri_nama_id = %s", (santri_id,))
    row = c.fetchone()
    if not row:
        c.execute("INSERT INTO angkatan (santri_nama_id) VALUES (%s)", (santri_id,))
        conn.commit()

    # Update nilai angkatan
    c.execute("UPDATE angkatan SET angkatan = %s WHERE santri_nama_id = %s", (value, santri_id))
    conn.commit()
    conn.close()
