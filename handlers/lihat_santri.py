...
async def handle_pilihan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "lanjutkan_santri":
        ...
        for idx, (id_h, nama_h) in enumerate(halaqah_list):
            cursor.execute("SELECT nama, hafalan FROM santri WHERE halaqah_id = ? ORDER BY nama ASC", (id_h,))
        ...
        # Buat tombol per halaqah
        keyboard = []
        for row in hasil:
          nama, ustadz = row
          label_halaqah = nama.replace("Halaqah_", "").replace("_", " ")
          label_ustadz = ustadz.strip()
          label = f"{label_halaqah} ({label_ustadz})"
...
async def tampilkan_santri_halaqah(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not santri:
        await query.message.reply_text(f"❌ Halaqah *{halaqah_nama}* tidak ditemukan atau belum ada santrinya.",
                                       parse_mode="Markdown")
        return ConversationHandler.END
