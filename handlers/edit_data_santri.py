...
async def simpan_edit(update: Update, context: CallbackContext):
    ...
    if field == "nama":
       c.execute("UPDATE santri_nama SET nama_lengkap = %s WHERE id = %s", (value, santri_id))
    elif field == "ttl":
    # Pisahkan value menjadi tempat dan tanggal
      if "," in value:
          tempat, tanggal = map(str.strip, value.split(",", 1))
          c.execute("UPDATE santri SET tempat_lahir = %s, tanggal_lahir = %s WHERE santri_nama_id = %s", (tempat, tanggal, santri_id))
      else:
      ...
      c.execute(f"UPDATE santri SET {field} = %s WHERE santri_nama_id = %s", (value, santri_id))
    conn.commit()
    conn.close()
