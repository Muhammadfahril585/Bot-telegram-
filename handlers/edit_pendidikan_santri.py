...
async def simpan_edit(update: Update, context: CallbackContext):
    ...
    c.execute("SELECT id FROM pendidikan WHERE santri_nama_id = %s", (santri_id,))
    row = c.fetchone()

    if not row:
        # Jika belum ada, buat dulu entri kosong
        c.execute("INSERT INTO pendidikan (santri_nama_id) VALUES (%s)", (santri_id,))
        conn.commit()

    # Update data
    c.execute(f"UPDATE pendidikan SET {field} = %s WHERE santri_nama_id = %s", (value, santri_id))
    conn.commit()
    conn.close()
