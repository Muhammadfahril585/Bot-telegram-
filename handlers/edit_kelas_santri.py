...
async def simpan_edit(update: Update, context: CallbackContext):
    ...
    c.execute("SELECT id FROM kelas WHERE santri_nama_id = ?", (santri_id,))
    row = c.fetchone()
    if not row:
        c.execute("INSERT INTO kelas (santri_nama_id) VALUES (?)", (santri_id,))
        conn.commit()

    # Update data kelas
    c.execute(f"UPDATE kelas SET {field} = ? WHERE santri_nama_id = ?", (value, santri_id))
    conn.commit()
    conn.close()
