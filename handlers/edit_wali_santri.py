...
async def simpan_edit(update: Update, context: CallbackContext):
    ...
    c.execute("SELECT id FROM wali WHERE santri_nama_id = %s", (santri_id,))
    row = c.fetchone()
    if not row:
        c.execute("INSERT INTO wali (santri_nama_id) VALUES (%s)", (santri_id,))
        conn.commit()

    # Update data wali
    c.execute(f"UPDATE wali SET {field} = %s WHERE santri_nama_id = %s", (value, santri_id))
    conn.commit()
    conn.close()
