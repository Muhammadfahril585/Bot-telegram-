...
from telegram.ext import (
    ConversationHandler, CallbackQueryHandler, CommandHandler, ContextTypes, MessageHandler, filters
)
...
async def mulai_lapor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📌 Silakan pilih halaqah yang ingin dilaporkan:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def pilih_santri(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        f"✅ Santri terpilih: *{santri_terpilih}*\nSilakan pilih status hafalan di bawah ini:",
        parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard))
    return PILIH_STATUS
...
async def pilih_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if status_terpilih == "hafalan_baru":
        ...
        db = get_db()
        cursor = db.cursor()
        santri = context.user_data["santri"]
        cursor.execute("SELECT hafalan FROM santri WHERE nama = %s", (santri,))
        result = cursor.fetchone()
        ...
        return await simpan_dan_lanjut(update, context)
...
async def input_juz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status = context.user_data.get("status", "hafalan_baru")
...
async def simpan_dan_lanjut(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if santri_list:
        ...
        keyboard = [
            [InlineKeyboardButton("📖 Hafalan Baru", callback_data="status|hafalan_baru"), InlineKeyboardButton("🔁 Murojaah", callback_data="status|murojaah")],
            [InlineKeyboardButton("📝 Ujian", callback_data="status|ujian"), InlineKeyboardButton("🕌 Simaan", callback_data="status|simaan")],
            [InlineKeyboardButton("🤒 Sakit", callback_data="status|sakit"), InlineKeyboardButton("📌 Izin", callback_data="status|izin")],
        ]
    ...
    for santri in santri_data:
        cursor.execute("""
            INSERT INTO laporan_pekanan (nama_halaqah, nama_santri, pekan_ke, bulan, hafalan_baru, status, total_juz, tanggal_laporan)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        """, (
            halaqah, santri["nama"], pekan, bulan,
            santri.get("halaman", 0), santri["status"], santri["total_juz"], tanggal
        ))
